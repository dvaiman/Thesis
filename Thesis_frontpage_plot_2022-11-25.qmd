...
#  geom_segment(aes(x=Ssyk2_hpi_scb_combined , y=-smoking*sign(smoking), xend=Ssyk2_hpi_scb_combined, yend=-5), fill="gold4", alpha=0.6) +
         geom_col(aes(x=Ssyk2_hpi_scb_combined , y=-pws*sign(smoking)), fill="gold4", alpha=0.7) + 
  geom_col(aes(x=Ssyk2_hpi_scb_combined , y=-smoking*sign(smoking)), fill="yellow", alpha=0.4) +
 ...
 # geom_text(aes(x="55",y=120, label = "Health"),angle=60, size=8, family="Roboto") +
  ...
  #                   ) +
  # text for percentage labels
      annotate(
        "text", 
           x = as.factor(rep(c(18,27,36,45, 55,77,84),times=4)), 
           y = rep(c(10, 30, 50, 60),each=7), 
           label = rep(c("*", "*", "*", "*", "2","1","9"),each=4), 
           color="grey60", 
           size=3 , 
           #angle=0, 
           fontface="bold", 
